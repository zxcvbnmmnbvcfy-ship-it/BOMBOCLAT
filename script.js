...
let activeMemeFilter = "all";

function renderMemes() {
  if (!memeGrid || typeof memeData === "undefined") return;

  const query = (memeSearch?.value || "").toLowerCase().trim();

  const filtered = memeData.filter(item => {
    const matchesSearch =
      item.title.toLowerCase().includes(query) ||
      item.category.toLowerCase().includes(query);

    const matchesCategory =
      activeMemeFilter === "all" || item.category === activeMemeFilter;
...
function openRandomMeme() {
  if (typeof memeData === "undefined" || !memeData.length) return;
  const item = memeData[Math.floor(Math.random() * memeData.length)];
  openLightbox(item);
}

if (memeSearch) memeSearch.addEventListener("input", renderMemes);

categoryButtons.forEach(btn => {
  btn.addEventListener("click", () => {
    categoryButtons.forEach(b => b.classList.remove("active"));
    btn.classList.add("active");
    activeMemeFilter = btn.dataset.filter;
    renderMemes();
  });
});

if (randomMemeBtn) randomMemeBtn.addEventListener("click", openRandomMeme);

renderMemes();


/* VIDEO DESTEKLI MEME ARSIVI - FIX */
function renderMemes() {
  if (!memeGrid || typeof memeData === "undefined") return;
  ...
  filtered.forEach(item => {
    ...
    if (isVideo) {
      card.innerHTML = `
        <video class="meme-video" muted loop playsinline preload="metadata">
          <source src="${item.src}" type="video/mp4">
        </video>
        <div class="video-hint">Tıkla: ses açılır</div>
        <div class="meme-info">
          <strong>${item.title}</strong>
          <span>${item.category}</span>
        </div>
